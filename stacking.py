...
if __name__ == '__main__':
    ...
    os.makedirs('submissions/stacking', exist_ok=True)
    ...
    methods = ['dtr', 'knn', 'ridge', 'lregression']

    models = []
    for i in range(1, N_ZONES+1):
        models_i = []
        for method in methods:
          filename = 'best_params/zone_{}/{}.txt'.format(i, method)
          file = open(filename, 'r')
          hyperparams = file.read().split()
          file.close()

          model = method_selection(method, hyperparams)
          model.fit(Xs[i-1][0], Ys[i-1]['TARGETVAR'])
          models_i.append(model)

        # use stacking on models
        estimators = [('dtr', models_i[0]), ('knn', models_i[1]), ('ridge', models_i[2]), ('lregression', models_i[3])]
        final_estimator = DecisionTreeRegressor(max_depth=5, min_samples_split=2, min_samples_leaf=1)
    ...
    for i in range(N_ZONES):
        ...
        Y_pred[i].to_csv(
            'submissions/stacking/Y_pred_Zone_{}.csv'.format(i+1), index=False)
